...
#By Continent
count_continent = df.groupby(['continent']).count()
count_continent[[0]].sort_values(by = 'id', ascending = False)

grouped_continent_mean = df.groupby(['continent']).mean()
grouped_continent_total = df.groupby(['continent']).sum()

print grouped_continent_mean
...
#Bar graph of Means by Continent
conMeans = grouped_continent_mean.loan
...
fig, ax = plt.subplots()
rects1 = ax.bar(ind, conMeans, width, color='b')

# add some text for labels, title and axes ticks
ax.set_ylabel('Mean Loan Size ($)')
ax.set_title('Mean Loan Size by Continent')
ax.set_xticks(ind + width/2)
ax.set_xticklabels(('Africa', 'Asia', 'Europe', 'North America', 'South America', 'USA'))
fig.autofmt_xdate()
...
#Logisitic Regression to fill in missing values of gender
